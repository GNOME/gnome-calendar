  ...
  for (gsize i = 0; events[i]; i++)
    {
      ...
      iter = g_sequence_insert_sorted (self->events,
                                       g_object_ref (events[i]),
                                       compare_events_cb,
                                       NULL);
      ...
      gtk_bitset_splice (bitset, position, 0, 1);
      gtk_bitset_add (bitset, position);
    }
  ...
  if (!gtk_bitset_is_empty (bitset))
    {
      ...
      guint n_added = gtk_bitset_get_size_in_range (bitset, first, last);

      gcal_event_list_items_changed (self, first, (last - first + 1) - n_added, n_added);
    }
}
  ...
  for (gsize i = 0; events[i]; i++)
    {
      ...
      iter = g_hash_table_lookup (self->event_to_iter, events[i]);
      if (!iter)
        continue;
  ...
  if (!gtk_bitset_is_empty (bitset))
    {
      ...
      gcal_event_list_items_changed (self, first, n_removed, (last - first + 1) - n_removed);
    }
}
