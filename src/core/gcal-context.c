                                 ...
                                 GVariant    *variant)
{
  ...
  if (weekday->value == G_DESKTOP_WEEKDAY_DEFAULT)
    weekday = g_enum_get_value (g_type_class_get (G_DESKTOP_TYPE_WEEKDAY), gcal_util_get_first_weekday_iso ());
...
gcal_context_class_init (GcalContextClass *klass)
{
  ...
  properties[PROP_WEEK_START_DAY] =
    g_param_spec_enum ("week-start-day", NULL, NULL,
                       G_DESKTOP_TYPE_WEEKDAY, gcal_util_get_first_weekday_iso (),
                       G_PARAM_READABLE | G_PARAM_EXPLICIT_NOTIFY | G_PARAM_STATIC_STRINGS);
...
{
  g_return_val_if_fail (GCAL_IS_CONTEXT (self), gcal_util_get_first_weekday_iso ());
